...
const buildFileName = (extension: "pdf" | "json") => {
...
export default function ExportAnalysisActions({
  ...
  const loadLogoDataUrl = async () => {
    if (logoDataUrlCacheRef.current !== undefined) {
      return logoDataUrlCacheRef.current;
    }
    ...
    return new Promise<string | null>((resolve) => {
      ...
      img.onload = () => {
        ...
        if (!ctx) {
          logoDataUrlCacheRef.current = null;
          resolve(null);
          return;
        }
        ...
        logoDataUrlCacheRef.current = dataUrl;
        resolve(dataUrl);
      };
