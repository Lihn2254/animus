...
const Brain3D = dynamic(() => import("../components/Brain3DModel"), {
  ssr: false,
  loading: () => (
    <div className="w-full h-96 bg-gradient-to-br from-slate-100 to-slate-50 rounded-2xl flex items-center justify-center">
...
export default function LandingPage() {
  return (
    <main className="min-h-screen bg-white px-6 py-8 sm:px-8 lg:px-16">
      {/* Header animado */}
      <header className="mx-auto flex max-w-7xl flex-col gap-8 rounded-3xl border border-slate-200 bg-white/80 px-6 py-6 shadow-2xl shadow-slate-900/10 backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between animate-fade-in-up">
        <div className="flex items-center gap-4">
          <img src="/logo_black.svg" alt="Animus Logo" className="h-16 w-16 flex-shrink-0 animate-float" />
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.32em] animate-slide-in-left" style={{ color: '#000123' }}>
              Animus
            </p>
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm"> 
          <Link
            href="/login"
            className="rounded-full border border-slate-300 bg-white/90 px-5 py-3 text-slate-700 transition hover:border-sky-400/70 hover:text-sky-700 hover:shadow-lg"
          >
            Iniciar sesión
          </Link>
          <Link
            href="/signup"
            className="rounded-full bg-sky-500 px-5 py-3 font-semibold text-white shadow-lg shadow-sky-500/30 transition hover:bg-sky-400 hover:shadow-xl"
          >
            Regístrate
          </Link>
        </div>
      ...
      <section className="mx-auto mt-10 max-w-7xl space-y-10">
        ...
        <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div className="space-y-6 bg-gradient-to-br from-sky-50 to-blue-50 p-8 rounded-4xl border border-sky-200 animate-slide-in-left stagger-1">
          ...
          <div className="rounded-[2rem] border border-slate-200 bg-white/90 p-4 shadow-2xl shadow-slate-900/10 animate-slide-in-right stagger-2">
            <div className="relative h-96 rounded-[1.5rem] overflow-hidden">
              <Brain3D />
              {/* Glow effect */}
              <div className="absolute inset-0 pointer-events-none rounded-[1.5rem] shadow-inner shadow-sky-400/30" />
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-10 lg:grid-cols-2 items-center">
          <div className="relative rounded-3xl overflow-hidden shadow-2xl h-80 animate-slide-in-left stagger-1">
          ...
          <div className="space-y-6 animate-slide-in-right stagger-2">
            <div className="bg-gradient-to-br from-emerald-50 to-teal-50 p-8 rounded-3xl border border-emerald-200">
              <div className="flex items-start gap-4 mb-4">
                <IconSparkles className="w-8 h-8 text-emerald-600 flex-shrink-0 mt-1" />
                <div>
                  <h4 className="text-lg font-semibold text-slate-900">Impacto Directo</h4>
                  <p className="text-slate-700 text-sm mt-1">
                    Monitoreo preventivo que alcanza instantáneamente a millones de estudiantes en México sin comprometer su privacidad.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-8 rounded-3xl border border-purple-200">
              <div className="flex items-start gap-4 mb-4">
                <IconShield className="w-8 h-8 text-purple-600 flex-shrink-0 mt-1" />
                <div>
                  <h4 className="text-lg font-semibold text-slate-900">Privacidad Garantizada</h4>
                  <p className="text-slate-700 text-sm mt-1">
                    100% anónimo. Cumplimiento total con LFPDPPP. Los estudiantes nunca son identificados, solo sus patrones de riesgo.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-8 rounded-3xl border border-blue-200">
              <div className="flex items-start gap-4 mb-4">
                <IconBolt className="w-8 h-8 text-blue-600 flex-shrink-0 mt-1" />
                <div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {stats.map((stat, idx) => (
            <div
              key={stat.label}
              className={`animate-fade-in-up stagger-${idx + 1} rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-50 to-blue-50 p-6 shadow-lg hover:shadow-xl transition text-center`}
            >
        ...
        <div className="rounded-[2rem] border-2 border-sky-300 bg-gradient-to-r from-sky-50/50 via-blue-50/50 to-indigo-50/50 p-10 shadow-xl">
          <div className="mb-8">
            ...
            {impacts.map((impact, idx) => {
              const getIcon = (iconName: string) => {
                ...
                switch(iconName) {
                  ...
                  case "hands": return <IconHands {...iconProps} />;
                  case "robot": return <IconRobot {...iconProps} />;
                  case "trending": return <IconTrendingUp {...iconProps} />;
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {values.map((item, idx) => {
            const colors = ['from-purple-50 to-purple-100', 'from-emerald-50 to-emerald-100', 'from-amber-50 to-amber-100'];
            const textColors = ['text-purple-700', 'text-emerald-700', 'text-amber-700'];
            return (
              <article
                key={item.title}
                className={`animate-fade-in-up stagger-${idx + 1} rounded-3xl bg-gradient-to-br ${colors[idx]} p-8 shadow-lg border border-slate-200 hover:shadow-xl transition hover:-translate-y-1`}
              >
        ...
        <section className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="space-y-6 rounded-3xl border border-slate-200 bg-white/90 p-8 shadow-xl shadow-slate-900/10 animate-slide-in-left">
            ...
            <div className="space-y-4 pt-4">
              {steps.map((step, idx) => {
                const bgColors = ['from-blue-50 to-cyan-50', 'from-emerald-50 to-teal-50', 'from-amber-50 to-orange-50'];
                return (
                  <div
                    key={step.label}
                    className={`animate-fade-in-up stagger-${idx + 1} rounded-3xl border border-slate-200 bg-gradient-to-r ${bgColors[idx]} p-5 hover:shadow-md transition hover:-translate-x-1`}
                  >
          ...
          <div className="rounded-3xl border border-slate-200 bg-white/90 p-8 shadow-xl shadow-slate-900/10 animate-slide-in-right">
            ...
            <ul className="mt-8 space-y-4">
              {technologies.map((tech, idx) => {
                ...
                const bgGradients = ['from-red-50 to-pink-50', 'from-green-50 to-emerald-50', 'from-blue-50 to-cyan-50', 'from-yellow-50 to-amber-50', 'from-purple-50 to-pink-50'];
                return (
                  <li key={tech} className={`animate-fade-in-up stagger-${idx + 1} flex gap-4 rounded-3xl border border-slate-200 bg-gradient-to-r ${bgGradients[idx]} p-5 hover:shadow-md transition hover:scale-105`}>
                    <span className="flex h-10 w-10 items-center justify-center rounded-full bg-white text-slate-600 flex-shrink-0 animate-pulse-slow">
                      {techIcons[idx]}
                    </span>
        ...
        <div className="rounded-[2rem] border-2 border-emerald-300 bg-gradient-to-r from-emerald-50 via-teal-50 to-cyan-50 p-10 shadow-xl">
          <div className="mb-8">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {marketData.map((data, idx) => (
            <div
              key={data.label}
              className={`animate-fade-in-up stagger-${idx + 1} rounded-2xl bg-gradient-to-br from-indigo-50 to-purple-50 p-6 shadow-lg border border-indigo-200 hover:shadow-xl hover:border-indigo-400 transition`}
            >
        ...
        {/* CTA Final */}
        <div className="rounded-[2rem] border border-sky-200 bg-gradient-to-r from-sky-50 via-blue-50 to-indigo-50 p-10 shadow-2xl shadow-sky-900/10 animate-fade-in-up">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-4 text-slate-600 leading-relaxed">
                Animus no es una solución genérica: combina contexto local, anonimato institucional, analítica predictiva y un flujo de alertas preventivas para apoyar el bienestar juvenil. Con crecimiento del 14.94% anual en el sector eHealth, es el momento para actuar.
              </p>
            </div>
      ...
      {/* Footer */}
      <footer className="mt-20 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white border-t border-slate-700">
